...
NAME=mirbsd10
ISO_URL=https://www.mirbsd.org/MirOS/current/cdrom10.iso
...
## Run KVM and install MirBSD 10
##
## NOTE:
## The installation ISO is pre-configured to use serial console,
## So we attach it.
kvm -name "$NAME" \
    -m "$RAM" \
    -net nic,model=e1000 \
    -net user \
    -boot cd \
    -drive file="$QCOW2_FILE",if=ide,media=disk,index=0 \
    -cdrom "$ISO_FILE" \
    -redir tcp:2222::22 \
    -serial mon:stdio
